...
%%
% Prepare files for Farber run
answer=input('Prepare a directory for a run on farber ? (0/1)  ');
if answer ~=0
      runfarber_name = input('Directory where files for farber are stored: ','s');
      eval(['!mkdir ',runfarber_name]);
      eval(['!mkdir ',runfarber_name,'/out']);
      eval(['!mkdir ',runfarber_name,'/tmp']);
      for il = 1:Nr_ext+1
          dirlev = ['Lev_',int2str(il)];
          eval(['!cp  ',problem_dir,'/',dirlev,'/*  ',runfarber_name]);
 %         eval(['!cp  run_mos2_l',int2str(il),'_hstruct.qs  ',runfarber_name]);
          fscript_generic=['run_l',int2str(il),'.qs'];
          fscript_name = [lower(runfarber_name),'_l',int2str(il),'.qs'];
          eval(['!cp  ',fscript_generic,'  ',runfarber_name,'/',fscript_name]);
          eval(['!rm -r ',problem_dir,'/Lev_',int2str(il)]);
      end
%       for il = 1:Nr_ext+1
%           eval(['!rm -r ',problem_dir,'/Lev_',int2str(il)]);
%           eval(['!rm -r ','Vacancies/Vac_lev_',int2str(il)]);
%       end
%     eval(['!mv ',runfarber_name,' ',problem_dir]);
      kstr = strfind(problem_dir,'/');
      problem_subdir = problem_dir(kstr+1:end);
      eval(['!cp -r ',problem_dir,'  ',runfarber_name]);
      %eval(['!mv ',problem_dir,'  ',runfarber_name]);
      farber_probdir = [runfarber_name,'/',problem_subdir];
      eval(['!cp  ',farber_probdir,'/problem_definition.mat  ',farber_probdir,'/../out/']);
      %
      % Create README for the directory of the run
      freadme = fopen([runfarber_name,'/README'],'w');
        fprintf(freadme,'*** Problem definition                       ***\n');
        fprintf(freadme,'   Number of levels:      %d  of %d\n',max_lev,Nr_ext+1);
      ...
      fclose(freadme);
      %
      % Create tar for transfer to farber
      eval(['!tar cvf ',runfarber_name,'.tar  ',runfarber_name]);
      eval(['!gzip  ',runfarber_name,'.tar']);
      fprintf(1,'All files ready for farber, sftp file %s\n',[runfarber_name,'.tar.gz']);
end
